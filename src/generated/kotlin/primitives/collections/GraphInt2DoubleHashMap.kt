...
import io.github.sooniln.fastcollect.ints.IntDoubleConsumer
...
@JvmInline
internal value class GraphInt2DoubleHashMap private constructor(@PublishedApi internal val map: Int2LongHashMap) : MutableInt2DoubleMap {
    ...
    override fun getOrDefault(key: Int, defaultValue: Double): Double = Double.fromBits(map.getOrDefault(key, defaultValue.toBits()))
    override fun put(key: Int, value: Double): Double = Double.fromBits(map.put(key, value.toBits()))
    public fun putIfAbsent(key: Int, value: Double): Double = Double.fromBits(map.putIfAbsent(key, value.toBits()))
    override fun set(key: Int, value: Double) { map[key] = value.toBits() }
    ...
    override fun foreach(action: IntDoubleConsumer) = map.foreach { key, value -> action.accept(key, Double.fromBits(value)) }
    override fun foreachKey(action: IntConsumer) = map.foreachKey(action)
    ...
    @JvmInline
    public value class Entry(@PublishedApi internal val entry: MutableInt2LongMap.MutableEntry) : MutableInt2DoubleMap.MutableEntry {
