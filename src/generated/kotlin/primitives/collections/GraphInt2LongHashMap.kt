...
internal class GraphInt2LongHashMap @JvmOverloads constructor(
    ...
    private inline fun set(key: Int, onAdd: () -> Long, onReplace: (slot: Int) -> Long) {
        require(key != EMPTY_KEY)
    ...
    override fun putAll(from: Int2LongMap) {
        if (from is GraphInt2LongHashMap && from.size / 2 > size) {
    ...
    private fun resetTo(from: GraphInt2LongHashMap) {
        check(!from.isEmpty())
    ...
    private fun setRehashing(keysArr: IntArray, valuesArr: LongArray, mask: Int, key: Int, value: Long) {
    ...
    private open inner class SlotIterator {
        private val keysArr = this@GraphInt2LongHashMap.keysArr
        private val valuesArr = this@GraphInt2LongHashMap.valuesArr
        ...
        fun updateValue(newValue: Long) {
            ...
            if (keysArr !== this@GraphInt2LongHashMap.keysArr) throw ConcurrentModificationException()
