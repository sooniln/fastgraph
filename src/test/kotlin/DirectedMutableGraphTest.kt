...
class DirectedMutableGraphTest {

    private val graph = Graphs.mutableGraph(true)
    ...
    fun removeVerticesWithIterator() {
        context(graph) {
            ...
            edgeProperty[e1] = "e1"
            val e2 = graph.addEdge(v2, v3).reference()
            edgeProperty[e2] = "e2"
            val e3 = graph.addEdge(v2, v2).reference()
            edgeProperty[e3] = "e3"
    ...
    fun mutateVertexWithEdges() {
        context(graph) {
            ...
            edgeProperty[e4] = "e4"
            val e5 = graph.addEdge(v1, v3).reference()
            edgeProperty[e5] = "e5"
            val e6 = graph.addEdge(v3, v3).reference()
            edgeProperty[e6] = "e6"

            graph.removeVertex(v2)
            assertThat(graph.vertices).containsExactlyInAnyOrder(v1.unstable, v3.unstable)
            assertThat(graph.edges).containsExactlyInAnyOrder(e5.unstable, e6.unstable)
    ...
    fun mutateEdge() {
        context(graph) {
            ...
            val e2 = graph.addEdge(v1, v1).reference()
