...
class UndirectedNetworkTest {
    ...
    fun outgoingEdges(immutable: Boolean) {
        ...
        assertThat(graph.outgoingEdges(v0).contains(e4)).isTrue
        ...
        assertThat(graph.outgoingEdges(v1).contains(e4)).isFalse
        ...
        assertThat(graph.outgoingEdges(v2).contains(e4)).isFalse
        ...
        assertThat(graph.outgoingEdges(v3).contains(e4)).isFalse
    ...
    fun incomingEdges(immutable: Boolean) {
        ...
        assertThat(graph.incomingEdges(v0).contains(e4)).isTrue
        ...
        assertThat(graph.incomingEdges(v1).contains(e4)).isFalse
        ...
        assertThat(graph.incomingEdges(v2).contains(e4)).isFalse
        ...
        assertThat(graph.incomingEdges(v3).contains(e4)).isFalse
    ...
    fun singleNeighborReachedByParallelEdges(immutable: Boolean) {
        ...
        assertThrows<IllegalStateException> { parallel.edge(a, b) }
        assertThat(parallel.edges(a, b)).hasSize(2)
    }
    ...
    fun edges(immutable: Boolean) {
        ...
        assertThat(valueGraph.edgeValues[e4]).isEqualTo(3.0f)
    }
    ...
    fun edgeOpposite(immutable: Boolean) {
        ...
        assertThat(graph.edgeOpposite(e4, v0)).isEqualTo(v0)
    ...
    fun edgeSourceAndTarget(immutable: Boolean) {
        ...
        assertThat(setOf(graph.edgeSource(e0), graph.edgeTarget(e0))).containsExactlyInAnyOrder(v0, v1)
        assertThat(setOf(graph.edgeSource(e1), graph.edgeTarget(e1))).containsExactlyInAnyOrder(v1, v2)
        assertThat(setOf(graph.edgeSource(e2), graph.edgeTarget(e2))).containsExactlyInAnyOrder(v0, v2)
        assertThat(graph.edgeSource(e3)).isEqualTo(v0)
        assertThat(graph.edgeTarget(e3)).isEqualTo(v0)
        assertThat(graph.edgeSource(e4)).isEqualTo(v0)
        assertThat(graph.edgeTarget(e4)).isEqualTo(v0)

        for (edge in listOf(e0, e1, e2, e3, e4)) {
            assertThat(graph.edgeSource(edge)).isEqualTo(graph.edgeSource(edge))
            assertThat(graph.edgeTarget(edge)).isEqualTo(graph.edgeTarget(edge))
            assertThat(graph.edgeOpposite(edge, graph.edgeSource(edge))).isEqualTo(graph.edgeTarget(edge))
            assertThat(graph.edgeOpposite(edge, graph.edgeTarget(edge))).isEqualTo(graph.edgeSource(edge))
        }
    }
    ...
    fun singularVertexAccessors(immutable: Boolean) {
        ...
        for (vertex in listOf(v0, v1, v2, v3)) {
            assertThrows<IllegalStateException> { graph.successor(vertex) }
            assertThrows<IllegalStateException> { graph.predecessor(vertex) }
            assertThrows<IllegalStateException> { graph.outgoingEdge(vertex) }
            assertThrows<IllegalStateException> { graph.incomingEdge(vertex) }
        }
    ...
    fun edgeToAndEdgesTo(immutable: Boolean) {
        ...
        assertThrows<IllegalStateException> { graph.edge(v0, v0) }
        assertThrows<IllegalStateException> { graph.edge(v0, v3) }
        assertThrows<IllegalStateException> { graph.edge(v3, v0) }
